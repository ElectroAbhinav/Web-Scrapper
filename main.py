from bs4 import BeautifulSoup
import requests
import time

print("Put some skills that you're not familiar with :->")
unfamiliar_skill = input('>')
print(f"Filtering Out: {unfamiliar_skill}")

def find_jobs():
    html_text = requests.get('https://www.timesjobs.com/candidate/job-search.html?searchType=personalizedSearch&from=submit&txtKeywords=python&txtLocation=&cboWorkExp1=0').text

    soup = BeautifulSoup(html_text, 'lxml')
    jobs = soup.find_all('li', class_='clearfix job-bx wht-shd-bx')
    for index, job in enumerate(jobs):
        time_posted = job.find('span', class_='sim-posted').span.text
        if 'few' in time_posted:
            company_name = job.find('h3', class_='joblist-comp-name').text.replace(' ', '')
            skills = job.find('span', class_='srp-skills').text.replace(' ', '')
            more_info = job.header.h2.a['href']
            if unfamiliar_skill not in skills:
                # Writing into a file the Jobs that are searched
                with open(f'posts/{index}.txt', 'w') as f:
                    f.write(f"Company Name: {company_name.strip()} \n")
                    f.write(f"Required Skills: {skills.strip()} \n")
                    f.write(f"More Information: {more_info} ")
                print(f'File Saved: {index}')


# Logic To Let Program Run Every 10 Minutes
if __name__ == '__main__':
    while True:
        find_jobs()
        time_wait = 10
        print(f"Waiting {time_wait} minutes to refresh...")
        time.sleep(time_wait * 60)


